...
void d_doc_lens_global_thread(uint16_t **d_doc_lens,
...
void malloc_global_thread(int **d_sort_index, float **d_sort_scores,
                          uint16_t **d_docs, size_t n_docs) {
  cudaSetDevice(0);
...
void doc_query_scoring_gpu_function(
  ...
  // nvtxRangePop();
  std::thread malloc_thread(malloc_global_thread, &d_sort_index, &d_sort_scores,
                            &d_docs, n_docs);
  std::thread copy_thread_1(d_doc_lens_global_thread, &d_doc_lens,
                            std::ref(lens), n_docs);
