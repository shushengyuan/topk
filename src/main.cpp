...
std::vector<std::string> getFilesInDirectory(const std::string& directory) {
  ...
  while ((dp = readdir(dirp)) != NULL) {
    ...
    if (S_ISREG(path_stat.st_mode))  // Check if it's a regular file - not a
                                     // directory
...
struct UserSpecifiedInput {
  ...
  UserSpecifiedInput(std::string qf, std::string df) { load(qf, df); }
  ...
  void load(std::string query_file_dir, std::string docs_file_name) {
    ...
    for (const auto& query_file_name : files) {
      ...
      query_file.close();
      ss.clear();
      std::sort(single_query.begin(),
                single_query.end());  // pre-sort the query
...
int main(int argc, char* argv[]) {
  if (argc != 4) {
    std::cout << "Usage: query_doc_scoring.bin <doc_file_name> "
                 "<query_file_name> <output_filename>"
              << std::endl;
    return -1;
  }
  ...
  ofs << std::chrono::duration_cast<std::chrono::milliseconds>(t3 - t2).count()
      << std::endl;
  // topk index
